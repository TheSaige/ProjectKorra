...
public class TerraSense extends EarthAbility implements AddonAbility, PassiveAbility {
    ...
    private HashMap<UUID, Long> lastSenseTimes = new HashMap<UUID, Long>();

    private ArrayList<LivingEntity> glowingEntities;

    public static double ShockwaveRadius = ConfigManager.getConfig().getDouble("Abilities.Earth.Shockwave.Range");
    public static boolean improvedWavesOnFallAndMove = ConfigManager.getConfig().getBoolean("ExtraAbilities.Hiro3.Earth.TerraSense.ImprovedWavesOnFallAndMove");
    public static boolean senseOnlyEarthBlocks = ConfigManager.getConfig().getBoolean("ExtraAbilities.Hiro3.Earth.TerraSense.SenseOnlyEarthBlocks");
    ...
    private double nonEarthMultiplier;
    ...
    public void setField() {
        ...
        isBlind = player.getPotionEffect(PotionEffectType.DARKNESS) != null ? 1 : (1/blindMultiplier);
        sandMultiplier = ConfigManager.getConfig().getDouble("ExtraAbilities.Hiro3.Earth.TerraSense.SandMultiplier");
        nonEarthMultiplier = ConfigManager.getConfig().getDouble("ExtraAbilities.Hiro3.Earth.TerraSense.nonEarthMultiplier");
        ...
        delayFlag = 0;
        senseDuration = ConfigManager.getConfig().getLong("ExtraAbilities.Hiro3.Earth.TerraSense.SenseDuration");
    }
    ...
    public void senseEarthbending() {
        ...
        for (EarthBlast eb : CoreAbility.getAbilities(EarthBlast.class)) {
            if (eb.getLocation() != null
                    ...
                    && eb.getLocation().distanceSquared(player.getLocation()) <= getCurrentRangeSqr()) {
        ...
        for (EarthSmash eb : CoreAbility.getAbilities(EarthSmash.class)) {
            if (eb.getLocations() != null && !eb.getLocations().isEmpty()
                    ...
                    && eb.getLocations().get(0).distanceSquared(player.getLocation()) <= getCurrentRangeSqr()) {
    ...
    public boolean isSenseable(LivingEntity le) {
        boolean senseDurationFlag = false;
        if (this.lastSenseTimes.containsKey(le.getUniqueId()) && System.currentTimeMillis() < this.lastSenseTimes.get(le.getUniqueId()) + this.senseDuration) {
            senseDurationFlag = true;
        }
        if (le.isOnGround()
                && player.getWorld().equals(le.getWorld())
                && player.getLocation().distanceSquared(le.getLocation()) <= getCurrentRangeSqr()) {
            this.lastSenseTimes.put(le.getUniqueId(), System.currentTimeMillis());
            return true;
        }
        if (!senseDurationFlag)
            this.lastSenseTimes.remove(le.getUniqueId());
        return senseDurationFlag;
    }

    @SuppressWarnings("deprecation")
    public boolean canSense() {
        boolean senseDurationFlag = false;
        if (this.lastSenseTimes.containsKey(player.getUniqueId()) && System.currentTimeMillis() < this.lastSenseTimes.get(player.getUniqueId()) + this.senseDuration) {
            senseDurationFlag = true;
        }

        if (EarthAbility.isSand(player.getLocation().clone().add(0, -0.2, 0).getBlock())) {
            this.isOnSand = 1; // Sand = baseline
        } else if (!EarthAbility.isEarthbendable(player, player.getLocation().clone().add(0, -0.2, 0).getBlock())) {
            this.isOnSand = 1 / this.nonEarthMultiplier; // Non-earthbendable = penalty
        } else {
            this.isOnSand = 1 / this.sandMultiplier; // Normal earthbendable = boosted
        }

        if (player.isOnGround()) {
            if (player.isSneaking() && player.getPotionEffect(PotionEffectType.DARKNESS) == null) {
                ...
                if (System.currentTimeMillis() < senseStartTime + senseDelayTime) {
                    if (!senseDurationFlag)
                        this.lastSenseTimes.remove(player.getUniqueId());
                    return senseDurationFlag;
                }
                this.lastSenseTimes.put(player.getUniqueId(), System.currentTimeMillis());
                return true;
            } else if (player.getPotionEffect(PotionEffectType.DARKNESS) != null) {
                isBlind = 1;
                this.lastSenseTimes.put(player.getUniqueId(), System.currentTimeMillis());
                return true;
            } else {
    ...
    public double getCurrentRangeSqr() {
        return getCurrentRange() * getCurrentRange();
    }
    ...
    public void load() {
        ...
        ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.Earth.TerraSense.ImprovedWavesOnFallAndMove", true);
        ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.Earth.TerraSense.SneakSenseDelay", 0);
        ConfigManager.getConfig().addDefault("ExtraAbilities.Hiro3.Earth.TerraSense.SenseDuration", 5000);
